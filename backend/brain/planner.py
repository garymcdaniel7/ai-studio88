...
def create_plan(user_request: str, context: dict | None = None) -> ExecutionPlan:
    ...
    tasks.append(
        Task(
            id=plan_task_id,
            name="Analyze Creative Intent",
            module="creative_session",
            action="plan",
    ...
    if any(
        w in request_lower
        for w in ["create", "generate", "photo", "portrait", "image", "luxury", "editorial"]
    ):
        gen_id = task_id()
        tasks.append(
            Task(
                id=gen_id,
                name="Generate Visual Content",
    ...
    if any(
        w in request_lower
        for w in ["reel", "video", "tiktok", "short", "trailer", "commercial", "clip"]
    ):
        vid_id = task_id()
        tasks.append(
            Task(
                id=vid_id,
                name="Create Video Production",
                module="video_studio",
                action="create",
                parameters={"description": user_request},
                depends_on=[plan_task_id],
    ...
    if any(
        w in request_lower for w in ["voice", "narration", "dialogue", "speak", "song", "music"]
    ):
        voice_id = task_id()
        tasks.append(
            Task(
    ...
    if any(
        w in request_lower
        for w in ["publish", "post", "schedule", "instagram", "tiktok", "youtube"]
    ):
        pub_id = task_id()
        tasks.append(
            Task(
