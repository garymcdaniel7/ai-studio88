...
class ExecutiveProducer:
    ...
    def analyze(self, context: dict) -> list[Insight]:
        ...
        insights.append(
            Insight(
                self.name,
                f"Production Budget: ~${jobs * 0.05:.2f}",
        ...
        if workers == 0:
            insights.append(
                Insight(
                    self.name,
                    "Production Bottleneck: No Workers",
    ...
    def analyze(self, context: dict) -> list[Insight]:
        ...
        if workers > 0 and vram_free >= 24:
            insights.append(
                Insight(
                    self.name,
                    "GPU Available: Route Immediately",
    ...
    def analyze(self, context: dict) -> list[Insight]:
        ...
        if failed_jobs > 0:
            insights.append(
                Insight(
                    self.name,
                    f"{failed_jobs} Failed Job(s) Detected",
