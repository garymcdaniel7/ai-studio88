...
class CreativeDirector(BaseAgent):
    ...
    def think(self, context: IntelligenceContext) -> AgentOutput:
        ...
        if any(w in idea.lower() for w in ["luxury", "hotel", "resort", "villa"]):
            scene = (f"Position {talent} in a luxury environment with warm golden-hour lighting. "
                     f"Shallow depth of field. 3/4 angle showing setting and subject.")
            reasoning_parts.append("Detected luxury/hospitality theme → warm lighting, elegant framing")
        elif any(w in idea.lower() for w in ["editorial", "rooftop", "city", "urban"]):
            scene = (f"Position {talent} against city skyline at blue hour. "
                     f"Dramatic side lighting, strong geometric lines from architecture.")
            reasoning_parts.append("Detected editorial/urban theme → dramatic lighting, architectural framing")
        elif any(w in idea.lower() for w in ["travel", "beach", "nature", "outdoor"]):
            scene = (f"{talent} in a natural environment with organic textures. "
                     f"Golden hour backlighting for a warm, aspirational feel.")
            reasoning_parts.append("Detected travel/nature theme → natural light, organic composition")
        else:
            scene = (f"Feature {talent} as focal point with complementary environment. "
        ...
        platform_direction = {
            "instagram": {"aspect": "4:5 portrait or 1:1 square", "style": "Bold colors, high contrast, scroll-stopping"},
            "tiktok": {"aspect": "9:16 portrait", "style": "High energy, movement-oriented, first-frame hook"},
            "youtube": {"aspect": "16:9 landscape", "style": "Cinematic, room for text overlays"},
            "pinterest": {"aspect": "2:3 portrait", "style": "Lifestyle aesthetic, soft tones, aspirational"},
            "website": {"aspect": "Custom/landscape", "style": "High resolution, hero-section quality"},
        }
