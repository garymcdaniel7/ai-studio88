...
class StoryDirector(BaseAgent):
    ...
    def think(self, context: IntelligenceContext) -> AgentOutput:
        ...
        if content_type in ("carousel", "story", "campaign"):
            ...
            recs.append({
                "title": "Narrative Arc",
                "content": "Each frame should tell a micro-story. "
                           "Hook → Build → Reveal → CTA.",
                "type": "narrative",
            })
        elif content_type in ("video", "reel"):
            recs.append({
                "title": "Video Structure",
                "content": f"5-second reel structure: Hook (0-1s) → Main (1-4s) → End (4-5s). "
                           f"Start with motion to stop scroll.",
