...
class VideoDirector(BaseAgent):
    ...
    def think(self, context: IntelligenceContext) -> AgentOutput:
        ...
        if "cinematic" in idea or "travel" in idea:
            camera = (
                "Slow dolly push-in with subtle parallax. Handheld micro-movement for organic feel."
            )
        elif "editorial" in idea or "fashion" in idea:
            camera = "Static tripod with subject movement. Clean, controlled framing."
        else:
            camera = (
                "Gentle pan or slow orbit around subject. Keep movement minimal and intentional."
            )
        ...
        recs.append(
            {
                ...
                "content": "Subtle movement: hair in breeze, fabric flow, gentle head turn. "
                "Avoid large movements that cause AI artifacts.",
                "type": "motion",
            }
        )
